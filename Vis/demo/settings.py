# Falsepyright (c) Facebook, Inc. and its affiliates.
...
default_sim_settings = {
    ...
    "scene": os.path.join(dir_path, "data/scene_datasets/habitat-test-scenes/skokloster-castle.glb"),
    # "scene": os.path.join(dir_path, "data/scene_datasets/habitat-test-scenes/van-gogh-room.glb"),
    "test_scene_data_url": "http://dl.fbaipublicfiles.com/habitat/habitat-test-scenes.zip",
    "goal_position": [5.047, 0.199, 11.145],
    "enable_physics": False,
    "enable_gfx_replay_save": False,
    "physics_config_file": os.path.join(dir_path, "./data/default.physics_config.json"),
    "num_objects": 10,
    "test_object_index": 0,
    "frustum_culling": True,
    # save video setting
    "save_video": True,
    "output_path": os.path.join(dir_path, "Vis/demo/video"),
}
...
def make_cfg(settings):
    ...
    def create_camera_3rd_spec(**kw_args):
        camera_sensor_spec = habitat_sim.CameraSensorSpec()
        camera_sensor_spec.sensor_type = habitat_sim.SensorType.COLOR
        camera_sensor_spec.resolution = [settings["height"] * 3, settings["width"] * 3]
        camera_sensor_spec.position = [0, 4, 0]
        camera_sensor_spec.orientation = [270, 0, 0]
        for k in kw_args:
            setattr(camera_sensor_spec, k, kw_args[k])
        return camera_sensor_spec

    if settings["color_3rd"]:
        color_sensor_spec_3rd = create_camera_3rd_spec(
            uuid="color_sensor",
            sensor_type=habitat_sim.SensorType.COLOR,
            sensor_subtype=habitat_sim.SensorSubType.PINHOLE,
        )
        sensor_3rd_specs.append(color_sensor_spec_3rd)

    if settings["depth_3rd"]:
        depth_sensor_spec = create_camera_3rd_spec(
            uuid="depth_sensor",
            sensor_type=habitat_sim.SensorType.DEPTH,
            channels=1,
            sensor_subtype=habitat_sim.SensorSubType.PINHOLE,
        )
        sensor_3rd_specs.append(depth_sensor_spec)

    if settings["semantic_3rd"]:
        semantic_sensor_spec = create_camera_3rd_spec(
            uuid="semantic_sensor",
            sensor_type=habitat_sim.SensorType.SEMANTIC,
            channels=1,
            sensor_subtype=habitat_sim.SensorSubType.PINHOLE,
        )
        sensor_3rd_specs.append(semantic_sensor_spec)
    ...
    # create 3rd view agent
    agent_3rdview_cfg = habitat_sim.agent.AgentConfiguration()
    agent_3rdview_cfg.sensor_specifications = sensor_3rd_specs

    agent_3rdview_cfg.action_space = dict(
    ...
    return habitat_sim.Configuration(sim_cfg, [agent_3rdview_cfg, agent_cfg]), settings
